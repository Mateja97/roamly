...
- Activity photos now appear on the detail screen's hero carousel. They were loading correctly but rendering at zero width, showing an empty gradient. (The full-screen photo viewer opened from that carousel is still affected and is being tracked separately.) ([#193](https://github.com/Mateja97/roamly/pull/193))
...
- The backend no longer refuses to start when its database already has a column a pending migration wanted to add. Previously this failed on startup and kept failing on every restart, with no way out except editing the database by hand. ([#196](https://github.com/Mateja97/roamly/pull/196))
