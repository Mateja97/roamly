...
// The Feed's own category/subtype pill-row state. Only `categories`/
// `subtypes` are actually read anywhere — `minRating`/`maxDistanceKm`
// moved to `ScopeDraft` and nothing reads them off `Filters`.
...
// subtype rail per category — Decision 5); a
...
export type ActivityListScreenProps = {
  ...
  // Feed is the app's home screen — App.tsx mounts this with no
