...
// ponytail: inlined rather than loaded from app/assets/getyourguide-logo.svg
// at runtime — same reason as TripadvisorLogo.tsx and Wordmark.tsx: Metro has
// no react-native-svg-transformer to import .svg files as components. The
// asset file stays as the source-of-record copy; this string mirrors its two
// paths exactly.
//
// INTERIM ASSET. Source: Wikimedia Commons, GetYourGuide_logo.svg
// ({{PD-textlogo}} — below the threshold of originality, so public domain for
// copyright; the page carries its own trademark notice). Trademark use here
// rests on Partner TCs 4.3.1, which licenses display of their Brand Elements.
//
// The provenance gap is real and deliberate: the Commons file lists "Unknown
// author" and a single 2025 third-party upload, so it can't be verified as
// GetYourGuide's current, unaltered mark — and 4.3.1(iv) forbids displaying an
// altered one. Accepted because 4.3.2 cures a breach by correcting it on
// request, making the downside a file swap. Replace with the official
// brand-kit asset when the partner portal supplies it; see
// docs/plans/getyourguide-brand-asset.md. Rendered as-is: both paths keep the
// asset's own #F53 fill, nothing is retinted.
const GETYOURGUIDE_LOGO_XML = `
...
// The mark's own proportions from its viewBox (382 wide / 302 tall) — a
// three-line stacked lockup, ~1.26:1, not a horizontal wordmark.
const ASPECT_RATIO = 382 / 302;

type GetYourGuideLogoProps = {
  /**
   * Rendered tall. Clears DESIGN_STANDARDS.md's >=20px partner-logo floor, but
   * that floor was written for Tripadvisor's ~4.7:1 horizontal mark. Three
   * stacked lines need far more: at 20px each line's cap height is under 5px.
   * 32px is the practical floor here.
   */
  height: number;
};

export function GetYourGuideLogo({ height }: GetYourGuideLogoProps) {
  return <SvgXml xml={GETYOURGUIDE_LOGO_XML} width={height * ASPECT_RATIO} height={height} />;
}
